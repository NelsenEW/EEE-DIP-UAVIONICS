...
/******************** NH Drone Pinout ***********************/
// LED pin. NOTE that the HIGH means the led is off, LOW means led is on.
#define blueLed 13
#define redLed 33
// MPU6050 I2C
#define I2C_SDA 22
...
// Pin definitions
int intPin = 21;  // These can be changed, 2 and 3 are the Arduinos ext int pins
...
void setup()
{
  ...
  // Set up the interrupt pin, its set as active low, push-pull
  pinMode(intPin, INPUT);
  attachInterrupt(digitalPinToInterrupt(intPin), interruptTrig, FALLING);
...
void loop()
{
  ...
  if (mpuInterrupt) { // check if data ready interrupt
    mpuInterrupt = false;
